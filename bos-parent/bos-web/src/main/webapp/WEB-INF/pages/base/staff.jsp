...
<script type="text/javascript">
	...
	function doDelete(){
		...
		if(rows.length==0){
			//没有选择记录，弹出提示
			$.messager.alert("提示信息","请选择删除对象","warning");
		}else{
			//选中了取派员，弹出确认框
			$.messager.confirm("删除确认","你确定要删除选中的取派员",function(r){
				if(r){
					var array=new	Array();
					//确定。发送请求
					//获取所有选中的取派员的id
					for (var i = 0; i < rows.length; i++) {
						var arrayb= rows[i];//json对象
						var	id=arrayb.id;
						array.push(id);
					}
					var	ids=array.join(",");
					location.href="StaffAction_deleteBath.action?ids="+ids;
				}
			});
		}
	}
...
<body class="easyui-layout" style="visibility:hidden;">
	...
	<div class="easyui-window" title="对收派员进行添加" id="addStaffWindow" collapsible="false" minimizable="false" maximizable="false" style="top:20px;left:200px">
		...
		<div region="center" style="overflow:auto;padding:5px;" border="false">
			<form	action="StaffAction_add.action"	id="addStaffFrom"	method="post">
